...
def traindd(input_wosrc, sx1, sz1, sx2, sz2, 
            tau_model1, tau_model2, 
            v_model, optimizer, epoch,
    ...
    for xz, sx1, sx2, taud1, taud_dx1, taud2, taud_dx2, t01, t0_dx1, t0_dz1, t02, t0_dx2, t0_dz2, index in data_loader:
        ...
        xzsic1 = torch.hstack((xzic1, sic1.view(-1,1)))

        # Compute T
        tau1 = tau_model1(xzsic1).view(-1)
        ...
        # Input for the data network
        xzsic2 = torch.hstack((xzic2, sic2.view(-1,1)))

        # Compute T
        tau2 = tau_model2(xzsic2).view(-1)

        # Compute v
        v1 = v_model(xzsic1[:, :2]).view(-1)
        v2 = v_model(xzsic2[:, :2]).view(-1)

        # Gradients
        gradient1 = torch.autograd.grad(tau1, xzsic1, torch.ones_like(tau1), create_graph=True)[0]
        tau_dx1 = gradient1[:, 0]
        tau_dz1 = gradient1[:, 1]

        gradient2 = torch.autograd.grad(tau2, xzsic2, torch.ones_like(tau2), create_graph=True)[0]
        tau_dx2 = gradient2[:, 0]
        tau_dz2 = gradient2[:, 1]
...
def training_loopdd(input_wosrc, sx1, sz1, sx2, sz2, 
                  tau_model1, tau_model2, v_model, optimizer, epochs,
                  batch_size=200**3,
    ...
    for epoch in range(epochs):
        ...
        # Train step
        mean_loss = traindd(input_wosrc, sx1, sz1, sx2, sz2, 
                  tau_model1, tau_model2, v_model, optimizer, epoch,
                  batch_size,
