...
def train3d(
    ...
    data_loader, ic = create_dataloader3d(
        input_wosrc,
        sx,
        sy,
        sz,
    ...
    for (
        ...
        xyzsic = torch.hstack(
            (xyz, sxic.view(-1, 1), syic.view(-1, 1), szic.view(-1, 1))
        )
        ...
        gradient = torch.autograd.grad(
            tau, xyzsic, torch.ones_like(tau), create_graph=True
        )[0]
        ...
        if args["exp_function"] == "y":
            ...
            rec_op_dz = torch.full_like(xz[:, 1], fill_value=0.0)
            ...
            rec_op_dz[mask] = (
                -args["exp_factor"]
                * torch.exp((xz[:, 1][mask]) ** args["exp_factor"])
                / (xz[:, 1][mask] ** (1 - args["exp_factor"]))
            )
        else:
            rec_op = xyz[:, 2]
            rec_op_dz = 1
        ...
        if args["factorization_type"] == "multiplicative":
            ...
            T_dy = (rec_op * tau_dy + taud_dy) * t0 + (rec_op * tau + taud) * t0_dy
            T_dz = (rec_op * tau_dz + rec_op_dz * tau) * t0 + (
                rec_op * tau + taud
            ) * t0_dz
        else:
        ...
        # Stationary causality
            ...
            wl2 = torch.exp(
                -args["causality_factor"]
                * torch.sqrt(
                    (xz[:, 0] - s) ** 2 + (xz[:, 1] - z[args["zid_source"]]) ** 2
                )
            )
        ...
        # Anti-causal causal non-stationary
            ...
            wl2 = torch.abs(
                ...
                - 1
            ) + (1 - torch.exp(torch.tensor(-5e-4 * epoch)))
...
def evaluate_tau3d(tau_model, grid_loader, num_pts, batch_size, device):
    ...
    with torch.no_grad():
        ...
        for i, X in enumerate(grid_loader, 0):
            ...
            xyzs = torch.hstack(
                (X[0], X[1].view(-1, 1), X[2].view(-1, 1), X[3].view(-1, 1))
            )
