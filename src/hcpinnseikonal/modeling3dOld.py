...
from hcpinnseikonal.utils import create_dataloader3dmodelingOld
...
def train3d(input_wosrc, sx, sy, sz,
          tau_model, v_sample, latent, optimizer, grid_size, num_pts,
    ...
    data_loader, ic = create_dataloader3dmodelingOld(
    ...
    for xyz, sx, sy, sz, t0, t0_dx, t0_dy, t0_dz, idx in data_loader:
        ...
        # Latent
        z = latent.repeat(sx.size()[0]).view(-1,latent.shape[0])
        ...
        # Compute v
        v = v_sample.view(128,-1,128).repeat(1, 32, 1).view(-1)[idx.long()]
...
#         pde = pde_lhs - 1 / (v ** 2)
        ...
        del idx, xyz, sxic, xyzsic, t0, t0_dx, t0_dy, t0_dz, ls, v, tau, tau_dx, tau_dy, tau_dz, gradient, pde
...
def evaluate_tau3d(tau_model, latent, grid_loader, num_pts, batch_size, device):
    ...
    with torch.no_grad():
        ...
        for i, X in enumerate(grid_loader, 0):
            ...
            xyzs = torch.hstack((X[0], X[1].view(-1,1), X[2].view(-1,1), X[3].view(-1,1), z.view(-1,1)))
...
def training_loop3d(input_wosrc, sx, sy, sz,
                    ...
                    device='cuda', wandb=None, args=None, num_vel=1):
    ...
    for vi in range(num_vel):
        ...
        for epoch in range(out_epochs):
            ...
            mean_loss = train3d(input_wosrc, sx, sy, sz,
                      tau_model, v_sample_i, latent_i, optimizer, grid_size, num_pts,
                      batch_size,
            ...
            if epoch % 250 == 0:
                print(f'Epoch {epoch + vi*out_epochs}, Loss {mean_loss:.7f}')
